...
module.exports={
    ...
    async execute(message,args){
        ...
        if(userData){
          ...
          let d = new Date();
          let n = d.getTime();
          let lastraceleaderboard;
          if(userData.lastraceleaderboard){
            lastraceleaderboard = userData.lastraceleaderboard;
          }else{
            lastraceleaderboard = 0;
          }
          let timeup;
          let timeup2;
          ...
          if(n - lastraceleaderboard >= timeup){
            console.log('here 1');
            let d2 = new Date();
            let n2 = d2.getTime();
            const response = await userModel.findOneAndUpdate({userID:message.author.id},{
              lastraceleaderboard:n2
            });
            ...
            message.guild.members.cache.forEach(async (member)=>{ 
              ...
              if(userData){
                ...
                if(value === message.guild.memberCount){
                    ...
                    user.sort((a,b)=>b.raceswon - a.raceswon);
                    user.forEach(async (e)=>{
                        value2 = value2 + 1;
                        if(value2 <= 10){
                            embed.addFields({name:`${value2}>${e.username}`,value:`${e.raceswon}`});
                        }
                      
                        if(value2 === user.length){
                            embed.setDescription(`(Based on races won)`)
            ...
            var msec = n - lastraceleaderboard;
            console.log(msec);
